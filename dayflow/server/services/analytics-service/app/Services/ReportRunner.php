...
final class ReportRunner
{
    ...
    private function documentExpiry(array $filters): array
    {
        ...
        foreach ($documents as $document) {
            ...
            $rows[] = [
                ...
                'department' => Payload::text($person, ['department_name', 'department'], ''),
                'document_type' => Payload::text($document, ['document_type', 'type', 'name'], 'Document'),
    ...
    {
        $query = ['scope' => AnalyticsScope::ORGANISATION]
            + $this->department($principal, AnalyticsScope::ORGANISATION, $filters);

        $enrolments = $this->mustHave(
            $this->downstream->collect('learning', '/enrolments', $query, 100, 20),
            'learning'
        );
        ...
        foreach ($enrolments as $enrolment) {
            $employeeId = Payload::text($enrolment, ['employee_id'], '');
            if ($employeeId === '') {
                continue;
            }

            $byEmployee[$employeeId] ??= ['assigned' => 0, 'mandatory' => 0, 'completed' => 0, 'overdue' => 0];
            $status = Payload::text($enrolment, ['status', 'enrolment_status'], '');
            $dueOn = Payload::text($enrolment, ['due_on', 'due_date'], '');

            $byEmployee[$employeeId]['assigned']++;

            if (Payload::bool($enrolment, ['is_mandatory', 'mandatory'])) {
    ...
    {
        $query = ['scope' => AnalyticsScope::ORGANISATION, 'status' => 'closed'];
