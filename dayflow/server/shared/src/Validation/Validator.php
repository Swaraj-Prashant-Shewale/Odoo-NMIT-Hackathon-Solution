...
final class Validator
{
    ...
    private function apply(string $field, string $rule, mixed $value): mixed
    {
        ...
        switch ($name) {
            ...
            case 'min':
                if (is_numeric($value) ? $value < (float) $argument : mb_strlen((string) $value) < (int) $argument) {
                    return $this->fail($field, is_numeric($value)
                        ? sprintf('%s must be at least %s.', $label, $argument)
                        : sprintf('%s must be at least %s characters.', $label, $argument));
                }
            ...
            case 'max':
                if (is_numeric($value) ? $value > (float) $argument : mb_strlen((string) $value) > (int) $argument) {
                    return $this->fail($field, is_numeric($value)
                        ? sprintf('%s must be no more than %s.', $label, $argument)
                        : sprintf('%s must be no more than %s characters.', $label, $argument));
                }
