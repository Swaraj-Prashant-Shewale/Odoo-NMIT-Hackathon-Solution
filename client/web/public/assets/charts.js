...
(function () {
    ...
    function line(spec, values, labels) {
        ...
        // The final x-axis label is centred on the last point; at right = 12
        // half of it was painted outside the viewBox.
        var left = 44, right = 34, top = 14, bottom = 28;
    ...
    function bar(spec, values, labels) {
        var width = 640, height = 220;
        var left = 44, right = 34, top = 14, bottom = 30;
        var plotW = width - left - right;
        var plotH = height - top - bottom;
        ...
        values.forEach(function (value, index) {
            ...
            if (labels[index] && shouldLabel(index, values.length)) {
                chart.appendChild(text(x + barWidth / 2, height - 8, truncate(labels[index], Math.max(6, Math.floor(slot / 5.5))), { size: 9 }));
            }
        });
    ...
    function stacked(spec, labels) {
        ...
        labels.forEach(function (label, index) {
            ...
            chart.appendChild(text(left + slot * index + slot / 2, height - 18, truncate(label, Math.max(6, Math.floor(slot / 5.5))), { size: 9 }));
        });
