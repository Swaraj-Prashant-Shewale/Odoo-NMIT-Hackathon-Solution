...
final class Kernel
{
    ...
    public function migrate(): void
    {
        ...
        try {
            ...
            // A schema that is not up to date is not safe to serve against, and
            // the marker is deliberately not written, so the next request tries
            // again rather than assuming the schema is ready.
            Logger::exception($exception, ['stage' => 'migrate']);

            throw $exception;
        }

        // The seed is demo and reference data, not structure, and it runs on
        // the request path. A failure here used to escape all the way out of
        // public/index.php - past Kernel::run()'s own handler - and become the
        // shutdown handler's "unrecoverable error", on every request, until
        // somebody edited the database by hand. One row a seed cannot write is
        // not a reason to stop answering; it is a reason to write a log line.
        $seeded = true;
        ...
        if (is_file($seeder)) {
            try {
                ...
                $seeded = false;
                Logger::exception($exception, ['stage' => 'seed']);
            }
        }

        // Marked only on a clean run, so an incomplete seed is retried on the
        // next request instead of being remembered as done.
        if ($seeded) {
     ...
     *
     * database/seed.php has to be in here as well as database/seeds/*.php. In
     * most services the file the kernel loads is a two-line forwarder and all
     * the actual data lives beside the migrations, so watching only the
     * forwarder meant editing a seed changed nothing: the digest still matched,
     * the marker still stood, and the new rows never appeared. The shared
     * roster in server/shared is watched for the same reason - it is the one
     * file every service's seed now reads its list of people from.
     */
    private function schemaFingerprint(): string
    {
        $patterns = [
            $this->basePath . '/database/migrations/*.sql',
            $this->basePath . '/database/seeds/*.php',
            $this->basePath . '/database/seed.php',
            Env::get('DAYFLOW_SHARED', '/var/www/shared') . '/src/Support/DemoCohort.php',
        ];
        ...
        foreach ($patterns as $pattern) {
            foreach (glob($pattern) ?: [] as $file) {
                // Qualified by directory: a service has both database/seed.php
                // and database/seeds/seed.php, and on basename alone the two
                // would collide and one of them would go unwatched again.
                $parts[] = $file . ':' . (filesize($file) ?: 0) . ':' . (filemtime($file) ?: 0);
            }
        }
