...
// Everyone in the demo company, from the shared roster. A notification names
// both the person and the account, because a preference is held by an account
// while the record it is about belongs to the person.
$employees = [];

foreach (DemoCohort::employeeIds() as $code => $employeeId) {
    $employees[$code] = [$employeeId, DemoCohort::userId($code)];
}
