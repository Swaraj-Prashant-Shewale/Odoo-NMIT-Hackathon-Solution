...
final class EnrolmentController
{
    ...
    public function index(Request $request): Response
    {
        ...
        // Whether this reads one person's enrolments or the company's.
        //
        // "scope" was previously accepted and silently discarded, so a caller
        // asking for the organisation was handed their own rows instead and
        // had no way to tell. Every company-wide training figure on the
        // platform was really one person's training. It now works, and it
        // needs learning.assign.any - the same permission that guards the
        // compliance report, which reads exactly the same records.
        $organisation = ($filters['scope'] ?? null) === 'all'
            && ($filters['employee_id'] ?? null) === null
            && $principal->can(Permissions::LEARNING_ASSIGN_ANY);
        ...
        if (!$organisation) {
            $builder->where('enrolments.employee_id', '=', $employeeId);
        }

        if (($filters['mandatory'] ?? null) === true) {
            $builder->where('courses.is_mandatory', '=', true);
        }
