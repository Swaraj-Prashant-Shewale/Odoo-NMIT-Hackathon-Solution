...
final class ReviewController
{
    ...
    public function index(Request $request): Response
    {
        ...
        $filters = Validator::make($request->query, [
            ...
            'scope' => 'nullable|in:to_complete,received,all,organisation',
            'status' => 'nullable|in:draft,submitted,acknowledged',
        ])->validated();

        // The organisation scope is the only one that reaches beyond the
        // caller, so it is the only one that needs a permission behind it.
        if (($filters['scope'] ?? null) === 'organisation' && !$principal->can(Permissions::TALENT_VIEW_ALL)) {
            unset($filters['scope']);
        }
