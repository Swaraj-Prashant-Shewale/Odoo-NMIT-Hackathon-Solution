...
final class AuthController
{
    ...
    public function register(Request $request): Response
    {
        ...
        if ($this->users->emailTaken($email)) {
            ...
            // Deliberately the same receipt a fresh sign-up gets. A different
            // one here would tell a stranger which addresses are registered.
            return $this->registrationReceipt(Env::bool('REQUIRE_EMAIL_VERIFICATION', false));
        }
        ...
        // Whether a new account has to confirm its address before it can sign
        // in. Off by default: on a deployment with no real mail server the
        // link goes to a log file, so insisting on it means nobody who signs
        // up can get in without an administrator fetching the link for them.
        // Turn it on wherever mail is genuinely delivered.
        $mustVerify = Env::bool('REQUIRE_EMAIL_VERIFICATION', false);

        $user = Connection::transaction(function () use ($data, $email, $passwordHash, $employeeCode, $role, $mustVerify): array {
            $created = $this->accounts->create([
                ...
                // Stamped at creation when verification is not being asked for,
                // because the column is also what login checks.
                'email_verified_at' => $mustVerify ? null : Clock::iso(),
            ], [$role], null);

            $token = $mustVerify ? $this->accounts->issueVerificationToken((string) $created['id']) : null;
        ...
        return $this->registrationReceipt($mustVerify);
    }
    ...
    public function login(Request $request): Response
    {
        ...
        if ($user['email_verified_at'] === null && Env::bool('REQUIRE_EMAIL_VERIFICATION', false)) {
    ...
    private function registrationReceipt(bool $verificationRequired = true): Response
    {
        return Response::created([
            'message' => $verificationRequired
                ? 'Thanks. If that address can be registered, a confirmation link is on its way to it.'
