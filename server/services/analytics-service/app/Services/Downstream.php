...
final class Downstream
{
    ...
    /**
     * Paths that were once expected of a service and are not routed by any of
     * them today. Every caller of these already has a fallback that derives
     * the same figure from a list endpoint, so the call is skipped rather than
     * made and failed: it saves several guaranteed round trips on every
     * dashboard view, and stops the log filling with 404s and 405s that are
     * not outages and would hide one that is.
     *
     * @var list<string>
     */
    private const UNROUTED = [
        '/attendance/summary',
        '/leave/summary',
        '/cost-by-department',
    ];
    ...
    {
        foreach (self::UNROUTED as $unrouted) {
            if (str_contains($path, $unrouted)) {
                return null;
            }
        }
