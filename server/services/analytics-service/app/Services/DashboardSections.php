...
final class DashboardSections
{
    ...
    {
        if ($this->principal->employeeId === null) {
            return null;
        }

        [$from, $to] = Clock::monthBounds(Clock::today());

        // Named explicitly. The attendance register widens to everything the
        // caller is allowed to see, so without an employee_id this card headed
        // "This month" quietly showed an HR administrator the whole company -
        // and only the first page of it, which is not even a company total.
        $query = [
            'employee_id' => $this->principal->employeeId,
            'from' => $from,
            'to' => $to,
        ];

        $rows = $this->downstream->collect('attendance', '/attendance', $query);

        if ($rows !== null) {
            return $this->summariseDays($rows) + ['period' => ['from' => $from, 'to' => $to]];
        }

        $summary = $this->downstream->record('attendance', '/attendance/summary', $query);
    ...
    public function attendanceWeek(): ?array
    {
        ...
        // "Your last seven days" has to be the viewer's seven days. Unscoped,
        // the register returned colleagues' rows and the last one to arrive for
        // each date won.
        $rows = $this->downstream->rows('attendance', '/attendance', [
            'employee_id' => $this->principal->employeeId,
            'from' => $from,
            'to' => $to,
            'per_page' => 50,
    ...
    public function leaveBalances(): ?array
    {
        ...
        foreach ($rows as $row) {
            // The leave service reports an annual quota plus the components it
            // is built from; none of them is called "entitled_days", so this
            // read used to be 0 and every balance rendered "N of 0 days left"
            // behind an empty progress bar.
            $entitled = Payload::float($row, ['annual_quota_days', 'entitled_days', 'allocated_days', 'opening_balance']);

            if ($entitled <= 0.0) {
                $entitled = Payload::float($row, ['opening_days'])
                    + Payload::float($row, ['accrued_days'])
                    + Payload::float($row, ['carried_forward_days'])
                    + Payload::float($row, ['adjusted_days']);
            }
            ...
            $totalAvailable += max(0.0, $available);
    ...
    public function pendingLeaveRequests(): ?array
    {
        ...
        // The card says "Nothing of yours is waiting on anybody" and links to
        // "All of my requests", so it has to be filtered to the viewer. The
        // leave register otherwise widens to whatever they may see, which for
        // an HR administrator is the entire company's pending queue.
        $query = [
            'employee_id' => $this->principal->employeeId,
            'status' => 'pending',
            'per_page' => 5,
    ...
    public function pendingApprovals(bool $canApproveLeave, bool $canApproveRegularisation, bool $canApproveExpense): ?array
    {
        ...
        if ($canApproveExpense) {
            // Without a scope the expense register defaults to the caller's
            // own claims, so this counted the approver's own submissions as
            // things awaiting their decision.
            $expenses = $this->downstream->total('payroll', '/expenses', [
                'scope' => 'approvals',
                'status' => 'submitted',
                'per_page' => 1,
                'page' => 1,
    ...
    {
        // /goals is hard-scoped to the caller and discards "scope" without
        // complaint, so this card used to be the viewer's own goals under a
        // heading that said "Team". The talent service has a real route for
        // the team view; use it.
        $rows = $this->downstream->rows('talent', '/goals/team', ['per_page' => 100]);

        if ($rows === null) {
            return null;
        }

        // The team route answers with one row per person, each carrying their
        // own goals - not a flat list of goals. Counting statuses on the outer
        // rows found none and drew an empty donut.
        $achieved = 0;
        $active = 0;
        $total = 0;

        foreach ($rows as $row) {
            $goals = is_array($row['goals'] ?? null) ? $row['goals'] : [$row];

            foreach ($goals as $goal) {
                $status = Payload::text($goal, ['status'], '');
    ...
    public function attendanceRateTrend(int $months = 6): ?array
    {
        ...
        foreach (Period::lastMonths($months) as $month) {
            ...
            $answered = true;
            $rate = $counters['attendance_rate'];

            // A month the register holds nothing for is a gap in the line, not
            // a month the company was 0% present. The client drops a null and
            // says how many months it left off; a 0.0 would be plotted.
            if ($rate === null) {
                $series[] = ['period' => $month['period'], 'label' => $month['label'], 'rate' => null];
                continue;
            }
    ...
    public function absenceByDepartment(int $limit = 5): ?array
    {
        ...
        // A department with no absence is not one of the five losing the most
        // days to it. Leaving them in filled the card with "0 days (0%)" rows
        // and hid its own "no absence has been recorded" state.
        $departments = array_values(array_filter(
            $departments,
            static fn (array $row): bool => $row['absent_days'] > 0
        ));
    ...
    {
        // The checklist endpoint takes no filters: it returns everyone with an
        // open checklist. Asking it twice with different status values got the
        // same answer twice, so the tile's two figures could never differ. One
        // read, and the split is made here from what each row actually says.
        $rows = $this->downstream->rows('employee', '/onboarding', ['per_page' => 100]);
        ...
        foreach ($rows as $row) {
            if (Payload::int($row, ['completed']) > 0) {
                $inProgress++;
            } else {
                $notStarted++;
            }
    ...
    public function documentsExpiring(int $days = 30): ?array
    {
        ...
        foreach ($rows as $row) {
            ...
            $documents[] = [
                ...
                'document_type' => Payload::text($row, ['title', 'document_type', 'type', 'name'], '')
                    ?: ucfirst(str_replace('_', ' ', Payload::text($row, ['category'], 'document'))),
    ...
    {
        $response = $this->downstream->record('learning', '/learning/compliance');

        // The learning service already works the organisation figure out, but
        // it reports it under "summary", not at the top level. Probing the top
        // level found nothing, fell through to the enrolment list - which is
        // hard-scoped to the caller and ignores both "scope" and "mandatory" -
        // and presented one person's own training as a company statistic.
        $summary = is_array($response['summary'] ?? null) ? $response['summary'] : null;

        if ($summary !== null && Payload::value($summary, ['completion_rate', 'compliance_rate', 'rate']) !== null) {
            return [
                'compliance_rate' => Payload::round(Payload::float($summary, ['completion_rate', 'compliance_rate', 'rate'])),
                'compliant' => Payload::int($summary, ['completed', 'compliant']),
                'total' => Payload::int($summary, ['obligations', 'total', 'required']),
            ];
        }
        ...
        // Defaulting the flag to true counted every enrolment as mandatory.
        // The list endpoint aliases the column as course_is_mandatory.
        $mandatory = array_values(array_filter(
            $enrolments,
            static fn (array $row): bool => Payload::bool($row, ['course_is_mandatory', 'is_mandatory', 'mandatory'], false)
        ));
    ...
    private function summariseDays(array $rows): array
    {
        ...
        // A month the register knows nothing about has an unknown rate, not a
        // rate of zero. Returning 0.0 put three flat zero months on the front
        // of the twelve-month trend line and suppressed the note that is
        // supposed to say how many months were left off it.
        $rate = $workingDays === 0 ? null : Payload::percent((float) $credited, (float) $workingDays);
