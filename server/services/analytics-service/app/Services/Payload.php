...
final class Payload
{
    ...
    public static function text(mixed $row, array $keys, string $default = ''): string
    {
        ...
        // Some services return a whole object where another returns a name:
        // the leave service answers "leave_type" with {id, name, code,
        // category}. Reading that as text used to yield the default, which is
        // how eight leave types came to be drawn as one slice labelled
        // "Leave". Naming the object is the closest honest reading of it.
        if (is_array($value)) {
